...
// class강의에서 배울때, 상속에서 부모 클래스에 해당되는 값이다.
...
/**
 * circular reference : 서로 참조하고 있다.
 */
console.log(IdolModel.prototype.constructor === IdolModel); //true가 나옴
...
console.log(yuJin2.hasOwnProperty('sayHello')); // '프로퍼티'가 고유의 프로퍼티인가? true
...
console.log(yuJin3.sayHello === wonYoung3.sayHello); // 프로토타입(상위)에 절약할 함수를 넣어주면 true가 나오게 된다.

console.log(yuJin3.hasOwnProperty('sayHello')); //false = 선언된 것이 아니라 상속받은 프로퍼티다.
...
console.log(ray.sayHello()); //이미 생성된 객체의 상속체인을 바꿀 수도 있다.

console.log(ray.constructor === FemaleIdolModel); //객체의 프로토를 IdolModel로 바꿨기 때문에, false.
